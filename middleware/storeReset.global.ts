...
export default defineNuxtRouteMiddleware(() => {
  const moduleGlob = import.meta.glob<true, string, { default: StoreDefinition }>(
    ['../store/**/*.ts'],
    { eager: true }
  )
  ...
  const targetStorePaths = Object.keys(moduleGlob)
    .filter((modulePath) => {
      return modulePath.includes(API_STORE_INDEX_FILE_PATH) || modulePath.includes(UI_STORE_INDEX_FILE_PATH)
    })
