...
const onAddBooksSubmit = async (): Promise<void> => {
  ...
  start()
  await apiStore.postBookSaveAll(
    props.user.uid,
    formData.value.map((data) => data.state)
  )
...
<template>
  <section :id="IMAGE_ANALYSIS_RESULT_HASH_ID" :class="cssModule['book-form-container']">
    ...
    <form novalidate @submit.prevent="onClickAddBookSubmit">
      ...
      <template v-if="userDetailBookPostResponse?.status === StatusCode.STATUS_CODE_CREATED">
        <UiPartsFeedbackAlert :class="cssModule['book-form-container__alert-text']" type="success" aria-live="assertive">
          書籍の登録が完了しました。
        </UiPartsFeedbackAlert>
      </template>
      ...
      <div :class="cssModule['book-form-container__action-button-container']">
        ...
        <UiPartsGeneralBasicButton type="button" color="processing" @click="uiStore.addFormStateRecord"
          >入力欄を追加</UiPartsGeneralBasicButton
        >
